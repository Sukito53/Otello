...
class UI(Tk):
    ...
    def update(self):
        for widget in self.winfo_children(): # ลบทั้งกระดาน
            if isinstance(widget, Button) and widget.cget("text") == '':
                widget.grid_forget()
        for i in range(8): # ยัดกระดานเข้าUI
            for j in range(8):
                if self.mode.array[i+1][j+1] in [0,1,-1]:
                    self.btn[i][j] = Button(self, text='',width=72,height=72,image=self.picture[self.mode.array[i+1][j+1]])
                else :
                    self.btn[i][j] = Button(self, text='',width=72,height=72,image=self.picture[2],command=lambda i=i+1, j=j+1: self.mode.runGame(i,j))
                self.btn[i][j].grid(row=i+1,column=j+1)

        self.Turn.grid_forget()
        self.numbw.grid_forget()
        self.Turn = Label(self,text='{} Turn'.format("White" if self.mode.lst_value[self.mode.round_] == -1 else ("Black" if self.mode.lst_value[self.mode.round_] == 1 else "empty")),font=self.custom_font)
        self.Turn.grid(row=1,column=9,padx=20)
        self.numbw = Label(self,text=f'{self.mode.showNumberOfBlackWhite()}',font=self.custom_font)
        self.numbw.grid(row=2,column=9)
    ...
    def show_winner(self):
        ...
        messagebox.showinfo("GAME END!!!",
            "{} WIN".format("White" if np.sum(self.mode.array == -1) > np.sum(self.mode.array == 1) else "Black"))
