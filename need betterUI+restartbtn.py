...
class PVP(BackEnd):
    ...
    # เตือนคะนิ้ง: อย่าลืม ลบ while True ใส่เพราะ รับ Input ตรงๆ ไม่งั้นเช็คwinพลาด
            ...
            main.update()
            if (self.checkWin(self.lst_value[not(self.round_)])):
                    print('in here we win')
                    main.show_winner()
                    # win
...
class UI(Tk):
    ...
    def set_value(self):
        ...
        self.textbtn = ['PVP','PVA Easy','PVA Normal','AVA','Home page','Restart'] # homepage + 2btn
    ...
    def home_page(self):
        self.clean_UI()
        self.grid_columnconfigure(0,weight=2)# using for make a button stick in middle
        self.label = Label(self,text="wasdwasd",font=self.custom_font)
        self.label.grid(row=0,column=0,sticky="NEW")

        for i in range(4):
            self.button = Button(self, text=self.textbtn[i],command=lambda num=i: self.getmode(num))
            self.button.grid(row=i+1,column=0,sticky="NEW")
    ...
    def create_board(self):
        ...
        self.realTime = Label(self,text=f'{self.mode.getDeltaTime()}',font=self.custom_font)
        self.realTime.grid(row=0,column=9,padx=20)
        self.after(1000,self.realtime)
        self.Turn = Label(self,text='{} Turn'.format("White" if self.mode.lst_value[self.mode.round_] == -1 else ("Black" if self.mode.lst_value[self.mode.round_] == 1 else "empty")),font=self.custom_font)
        self.Turn.grid(row=1,column=9,padx=20)
        self.numbw = Label(self,text=f'{self.mode.showNumberOfBlackWhite()}',font=self.custom_font)
        ...
        for i in range(4,6):# 2 btn
            self.button = Button(self, text=self.textbtn[i],command=lambda i=i : self.ingame(i))
            self.button.grid(row=i+2,column=9)

        for i in range(8):#board
            for j in range(8):
                self.btn[i][j] = Button(self, text='',width=72,height=72,image=self.picture[3])
    ...
    def start_game(self):
        ...
        if 'AVA_Easy' in str(self.mode):
            ...
            self.mode.valid_move = self.mode.preview(self.mode.lst_value[self.mode.round_])
        ...
        for i in range(8): # ยัดกระดานเข้าUI
            for j in range(8):
                if self.mode.array[i+1][j+1] in [0,1,-1]:
                    self.btn[i][j].config(image=self.picture[self.mode.array[i+1][j+1]])
                else :
                    self.btn[i][j].config(image=self.picture[2],command=lambda i=i+1, j=j+1: self.mode.runGame(i,j))

        self.Turn.config(text='{} Turn'.format("White" if self.mode.lst_value[self.mode.round_] == -1 else ("Black" if self.mode.lst_value[self.mode.round_] == 1 else "empty")))
    ...
    def show_winner(self):
        ...
        messagebox.showinfo("GAME END!!!","{} WIN".format("White" if np.sum(self.mode.array == -1) > np.sum(self.mode.array == 1) else "Black"))
